...
- **문제 상황**: 답변 화면에서 "별표 1"을 "제16조"로 잘못 인용하는 게 발견됨
...
### 7. "달걀 껍질은 음식물쓰레기인가요?" 오답

- **문제 상황**: "달걀"로 검색하면 정상 답변, "계란"으로 검색하면 다른 답변이 나옴. 또한 문서에 "달걀 껍질 = 일반쓰레기"라고 명확히 적혀 있는데 "음식물쓰레기로 배출해야 합니다"라고 오답
- **원인 분석**: 두 가지 원인이 겹쳐 있었음
  1. **동의어 미매칭**: 문서에 "계란"만 있고 "달걀"이 없어서 임베딩 유사도 점수 차이가 발생
  2. **LLM의 섹션 제목 오해석**: 근거 문서의 섹션 제목이 `[음식물인데 일반쓰레기인 것들]`인데, LLM이 "음식물"이라는 단어에 끌려서 내용과 반대로 해석
- **해결**: 두 곳을 수정
  1. **프롬프트 규칙 강화** — "음식물인데 일반쓰레기" 같은 제목을 정확히 해석하라는 지시와 달걀 껍질 관련 Few-shot 예시 추가
  2. **가이드 문서 수정** — 섹션 제목을 `[음식물처럼 보이지만 일반쓰레기(종량제봉투)로 배출해야 하는 것들]`로 명확하게 변경, 각 항목에 `→ 일반쓰레기` 표기 추가, "달걀(계란)" 동의어 병기
- **결과**: 프롬프트·문서 수정 후 통과율 83.3% → 93.3%, 환각률 13.3% → 6.7%로 개선

### 8. 인기 질문 집계가 동일 문장만 묶음

- **문제 상황**: 관리자 대시보드의 인기 질문 TOP N이 실제 인기와 맞지 않음. "종이컵 버리는 법"과 "종이컵은 어떻게 버려요?"가 같은 질문인데 별개로 집계됨
- **원인 분석**: 기존 방식이 `GROUP BY question`(SQL 문자열 완전 일치)으로 집계해서, 표현만 다르고 의미가 같은 질문을 별개로 카운트. 사용자마다 같은 의도를 다르게 표현하므로 실제 인기 주제가 분산되어 보임
- **해결**: 임베딩 기반 클러스터링 도입. 질문이 들어올 때마다 임베딩 벡터를 구한 뒤 기존 클러스터 벡터와 코사인 유사도를 비교, 0.85 이상이면 기존 클러스터에 편입하고 미만이면 새 클러스터 생성. `question_clusters` 테이블에 대표 질문·벡터·카운트를 저장
- **결과**: 의미가 같은 질문이 하나로 묶여 실제 인기 주제를 정확히 반영. 기존 방식은 클러스터가 없을 때의 폴백으로 유지
...
**참고**: 이전 버전(정답셋 25문항, 의미 유사도 기반 채점)에서는 통과율 80.0%·검색 정확도 81.0%로 측정됐으나, 평가 방식과 정답셋 규모가 바뀌어 위 수치로 대체됨. 상세 실험 과정은 `산출물/RAG 시스템 평가 및 개선 실험 보고서.md` 참고.
