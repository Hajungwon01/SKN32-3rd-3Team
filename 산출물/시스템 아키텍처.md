...
graph TD
    subgraph 외부 클라이언트 / 평가
        Browser[브라우저]
        Eval[평가 스크립트 <br/> evals/run_eval.py]
    end
...
* **`run_eval.py`**: FastAPI 서버를 거치지 않고 `rag_service`를 직접 호출하여 답변 품질을 측정하는 평가 스크립트입니다.
...
|  | GET | `/api/documents` | 문서 목록 조회
 ...
 |
|  | GET | `/api/documents/tree` | 문서 트리 구조 조회
...
| **챗봇 / RAG (`rag.py`)** | POST | `/api/chat` | 챗봇 메인 대화
 ...
 |
|  | GET | `/api/chat/sessions` | 대화방 목록 조회 및 복원
...
|  | GET | `/api/rag/status` | 인덱스 상태 확인
 ...
 |
|  | GET | `/api/popular-questions` | 인기 질문 TOP N 조회

 |
| **관리자 (`admin.py`)** | GET | `/api/admin/stats` | 전체 통계 조회

 |
|  | GET | `/api/admin/top-questions` | 자주 묻는 질문 조회

 |
|  | GET | `/api/admin/region-stats` | 지역별 질문 분포 조회

 |
|  | GET | `/api/admin/daily-trend` | 일별 질문 추이 조회

 |
|  | GET | `/api/admin/documents` | 인덱싱된 문서 목록 조회

 |
|  | POST | `/api/admin/upload` | 문서 파일 업로드 및 인덱싱
