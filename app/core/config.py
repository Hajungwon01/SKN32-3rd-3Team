...
class Settings(BaseSettings):
    ...
    # 답변 생성 시 문서 종류별로 확보할 자리 수.
    # 법령은 조문 수가 많아 청크 비중이 가이드를 압도한다(약 90:10).
    # 유사도 순으로만 뽑으면 "종이컵 어떻게 버려요?" 같은 실생활 질문에도
    # 법령 조문만 검색되어 정작 필요한 배출 방법을 놓친다.
    # 가이드(실천 방법)와 법령(법적 근거) 자리를 각각 보장해야 두 층 답변이 된다.
    # 가이드 자리는 다시 "지역 전용"과 "전국 공통"으로 나눈다.
    # 공통 가이드(에너지·탄소중립·일회용품 등)가 늘어나면서
    # 가이드 자리를 공통이 모두 차지해 지역 문서가 밀려나는 문제가 있었다.
    # (예: "쓰레기 몇 시에 내놔요?" → 부산 배출시간 청크가 검색에서 탈락)
    RAG_TOP_K_REGION: int = 2  # 선택한 지역 전용 문서
    RAG_TOP_K_COMMON: int = 1  # 전국 공통 가이드
    RAG_TOP_K_LAW: int = 2     # 법령

    # 지역+공통 합계. (하위 호환용 — 지역 필터가 없을 때 쓰인다)
    RAG_TOP_K_GUIDE: int = 3
